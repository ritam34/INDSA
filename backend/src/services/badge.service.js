...
class BadgeService {
  ...
  async getAllBadges(filters = {}) {
    const { type, rarity, isActive = true } = filters;
  ...
  async getUserBadges(userId, options = {}) {
    ...
    if (includeProgress) {
      ...
      return {
        earned: userBadges.map(ub => ({ ...ub, earned: true, progress: 100 })),
  ...
  async awardBadge(userId, badgeId, metadata = {}) {
    ...
    await prisma.notification.create({
      data: {
        ...
        message: `Congratulations! You've earned the "${userBadge.badge.name}" badge!`,
        metadata: {
          badgeId,
  ...
  async checkAndAwardBadges(userId) {
    ...
    for (const badge of availableBadges) {
      const isEligible = await this.checkBadgeEligibility(userId, badge, userStats);
  ...
  checkProblemSolvingCriteria(userStats, criteria) {
    if (criteria.totalProblems && userStats.totalSolved < criteria.totalProblems) {
      return false;
    }
    if (criteria.easyProblems && userStats.easyProblemsSolved < criteria.easyProblems) {
      return false;
    }
    if (criteria.mediumProblems && userStats.mediumProblemsSolved < criteria.mediumProblems) {
      return false;
    }
    if (criteria.hardProblems && userStats.hardProblemsSolved < criteria.hardProblems) {
      return false;
    }
    return true;
  }
  ...
  async checkCommunityCriteria(userId, criteria) {
    ...
    if (criteria.helpfulVotes) {
      ...
      if ((totalUpvotes._sum.upvotes || 0) < criteria.helpfulVotes) return false;
    }
  ...
  async createBadge(badgeData) {
    const badge = await prisma.badge.create({
      data: {
        name: badgeData.name,
        slug: badgeData.slug || badgeData.name.toLowerCase().replace(/\s+/g, '-'),
